...
let SSHQuickConnect = class SSHQuickConnect extends PanelMenu.Button {
  ...
  _init() {
    ...
    this.add_child(icon);
    let sshConfig = GLib.spawn_command_line_sync("cat " + GLib.get_home_dir() + "/.ssh/config");
    let hostString = sshConfig[1];
    if (hostString instanceof Uint8Array) {
      hostString = imports.byteArray.toString(hostString);
    }
